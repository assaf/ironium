...
module.exports = class Beanstalk {
      ...
      .then(client => {
        ...
        return new Promise((resolve, reject) => {
          ...
          function callback(error) {
            ...
            if (error) {
              debug('%s: %s => %s', session.id, command, error);
              reject(new QueueError(error));
            } else if (results.length > 1) {
              debug('%s: %s => %j', session.id, command, results);
              resolve(results);
            } else {
              debug('%s: %s => %j', session.id, command, results[0]);
              resolve(results[0]);
            }
          }
  ...
  del(messageID, options, callback) {
    return this.request('destroy', messageID)
      .then(() => callback(), callback);
  }

  msg_release(messageID, reservationID, options, callback) { // eslint-disable-line camelcase
    const priority = 0;
    return this.request('release', messageID, priority, options.delay)
      .then(() => callback(), callback);
  }
