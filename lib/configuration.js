...
module.exports = class Configuration {
  ...
  constructor(source) {
    ...
    if (source.ironio && source.ironio.token && source.ironio.projectID) {
      ...
      this.authenticate = Util.format(IRONIO_AUTHENTICATE, source.ironio.token, source.ironio.projectID);
      this._webhookURL  = Util.format(IRONIO_WEBHOOKURL, hostname, source.ironio.projectID, source.ironio.token);
      ...
      this.authenticate = Util.format(IRONIO_AUTHENTICATE, process.env.IRON_MQ_TOKEN, process.env.IRON_MQ_PROJECT_ID);
