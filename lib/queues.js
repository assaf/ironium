...
class Queue extends EventEmitter {
  ...
  _delayJob(job, duration) {
    ...
    ++this._queuing;
		...
		return new Promise((resolve, reject)=> {
			this._putClient.post({ body, timeout, delay }, (error, jobID)=> {
				if (error)
					...
					this._notify.debug('Queued job %s on queue %s', jobID, this.name, body);
					resolve(jobID);
				}
  ...
  stop() {
    this._processing = false;
		if (this._getSession) {
      this._getSession.end();
			this._getSession = null;
		}
		if (this._putSession) {
      this._putSession.end();
  ...
  _reserveAndProcess() {
		const session = this._getClient;
		const reserveAsync = new Promise(function(resolve, reject) {
			const timeout = 0;
		...
		return reserveAsync
			.then((message)=> {
				if (message) {
					...
					return this._runAndDestroy(session, message_id, reservation_id, payload)
  ...
  _startProcessing() {
    for (let i = 0; i < this._width; ++i)
			this._processContinuously(this._getClient);
  }
  ...
  _processContinuously(session) {
    ...
    if (!(queue._processing && queue._handlers.length))
      return;
		...
		const timeout = msToSec(RESERVE_TIMEOUT);
  ...
  _runAndDestroy(session, message_id, reservation_id, payload) {
		...
		return Promise.resolve()
			...
			.catch((error)=> {
				...
				const releaseAsync = new Promise((resolve)=> {
					session.msg_release(message_id, reservation_id, { delay }, (releaseError)=> {
						if (releaseError)
			...
			.then(()=> {
				return new Promise((resolve)=> {
					session.del(message_id, { reservation_id }, (deleteError)=> {
						// Remove job from queue: there's nothing we can do about an error here
						if (deleteError)
							this._notify.info('Could not delete job %s:%s', this.name, jobID, deleteError);
						resolve();
					});
				});
			});
  ...
  purgeQueue() {
    ...
    // return any jobs.
    const session = this._putClient;

		function peekAndDelete() {
			return new Promise(function(resolve, reject) {
				session.peek({ n: 100 }, function(error, messages) {
					if (error && /not found/i.test(error))
						...
						const deleteAll = Promise.all(ids.map(deleteAsync));
						const andRepeat = deleteAll.then(peekAndDelete);
						resolve(andRepeat);
					} else
						resolve();
				});
		...
		function deleteAsync(message_id) {
			return new Promise(function(resolve, reject) {
				session.del(message_id, {}, function(error) {
					if (error)
						reject(error);
					else
						resolve();
  ...
  get _putClient() {
		...
		if (this._config.isBeanstalk) {
			if (!this._putSession) {
				// Setup: tell Beanstalkd which tube to use (persistent to session).
				const session   	= new Beanstalk(this._server, `${this.name}/put`, setupSession);
  ...
  // blocked, so all other operations should happen on the _putClient session.
  get _getClient() {
		const tubeName  = this._prefixedName;
		if (this._config.isBeanstalk) {
			if (!this._getSession) {
				// Setup: tell Beanstalkd which tube we're watching (and ignore default tube).
				const session  		= new Beanstalk(this._server, `${this.name}/get`, setupSession);
	...
	get ironMQ() {
		if (!this._ironMQ) {
			...
			this._ironMQ = new ironMQ.Client({
				project_id: config.project_id,
				token:      config.token,
				host:       config.host,
				queue_name: this._prefixedName
			});
		}
		return this._ironMQ;
	}
