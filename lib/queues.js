...
class Session {
  ...
  // Close this session
	...
	post(payload, options, callback) {
		const priority = 0;
		return this.request('put', priority, options.delay, options.timeout, payload)
...
class Queue extends EventEmitter {
  ...
  _delayJob(job, duration) {
    ...
    ++this._queuing;
		...
		return new Promise((resolve, reject)=> {
			this._put.post(payload, { timeout, delay }, (error, jobID)=> {
				if (error)
    ...
    const session = this._put;
		...
		function peekAndDelete() {
			return new Promise(function(resolve, reject) {
				session.peek({ n: 100 }, function(error, ids) {
					if (error)
						reject(error);
					else if (ids.length) {
