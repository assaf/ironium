...
class Queue {
      ...
      return new Promise(function(resolve, reject) {
        client.post(messages, function(error, jobIDs) {
          ...
          if (error)
            ...
            reject(new Error(`Error queuing to ${self.name}: ${error.message}`));
          else {
            jobIDsArray.forEach(function(jobID, index) {
              const body = messages[index].body;
              debug('Queued job %s on queue %s', jobID, self.name, body);
            });
            resolve(jobIDsArray);
  ...
  _runAndDestroy(client, job) {
    ...
    function runHandlers() {
      ...
      debug('Processing queued job %s:%s', self.name, jobID, body);
    ...
    function logProcessingTime() {
      ...
      debug('Completed queued job %s:%s in %s', self.name, jobID, elapsed);
    }

    function deleteJob() {
      const reservationID = job.reservation_id;
      client.del(jobID, { reservation_id: reservationID }, function(deleteError) { // eslint-disable-line camelcase
        // Remove job from queue: there's nothing we can do about an error here
        if (deleteError)
          debug('Could not delete job %s:%s', self.name, jobID, deleteError);
      });
    }
