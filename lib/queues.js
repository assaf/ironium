...
class Queue extends EventEmitter {
  ...
  _delayJob(job, duration) {
    ...
    ++this._queuing;
		...
		return new Promise((resolve, reject)=> {
			this._put.post({ body, timeout, delay }, (error, jobID)=> {
				if (error)
  ...
  get _put() {
    ...
    if (!this._putSession) {
      ...
      const session   = this._newSession('put', setupSession);
  ...
  _reserve(index) {
    ...
    if (!this._reserveSessions[index]) {
      ...
      const session   = this._newSession(index, setupSession);
	...
	_newSession(suffix, setup) {
		const config       = this._config;
		const isBeanstalkd = config.port;
		if (isBeanstalkd)
			return new Session(this._server, `${this.name}/${suffix}`, setup);
		else
