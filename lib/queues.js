  ...
  _processContinuously(client) {
    if (this._processing) {
      ...
      this._server.getReserveCapacity()
        .then(maxJobs => {
          debug('Waiting for jobs on queue %s (at most %s)', this.name, maxJobs);
          return this._reserveJobs(client, wait, maxJobs);
        })
        .then(jobs => {
          // If we were requested to shut down while waiting for jobs,
          // same behavior as when over capacity: try to release.
          if (this._processing) {
            return this._server.incrementActiveJobs(jobs.length)
              .then(capacity => [ jobs, capacity ]);
          } else
            return [ jobs, 0 ];
        })
        .then(([ jobs, capacity ]) => {
          // We may have reserved more than the currenty capacity.
          ...
          jobsToRun.forEach(job => {
            this._runAndDestroy(client, job)
              .catch(() => { })
              .then(() => this._server.decrementActiveJobs(1));
          });
...
module.exports = class Queues {
  ...
  incrementActiveJobs(count) {
    return this.configPromise
      .then(config => {
        const capacity    = config.concurrency - this._activeJobs;
        const increment   = Math.min(capacity, count);
        this._activeJobs += increment;
        return increment;
      });
  }

  decrementActiveJobs(count) {
    this._activeJobs -= count;

    if (this._waitingForReserve.length) {
      this._calculateReserveCapacity()
        .then(capacity => {
          if (this._waitingForReserve.length && capacity > 0) {
            const resolve = this._waitingForReserve.shift();
            resolve(capacity);
          }
        });
    }
