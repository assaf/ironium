...
// Ignored in test environment (IronMQ API, seconds).
const RELEASE_DELAY         = 60;

// How long to wait when reserving a job (IronMQ API, seconds).
const RESERVE_WAIT          = 30;
...
class Queue {
  ...
  delayJob(job, duration) {
    ...
    const queue   = this;
    ++queue._queuing;
    return queue._putClientPromise.then(postJob);
  ...
  _processContinuously(client) {
    ...
    if (this._processing && hasHandlers) {
      ...
      const wait    = (process.env.NODE_ENV === 'test' ? 0 : RESERVE_WAIT);
    ...
    function releaseJob(job) {
      const jobID          = job.id;
      const reservation_id = job.reservation_id;
      client.msg_release(jobID, reservation_id, { delay: 0 });
    }
  }
  ...
  _runAndDestroy(client, job) {
    ...
    function deleteJob() {
      const reservation_id = job.reservation_id;
      client.del(jobID, { reservation_id }, function(deleteError) {
    ...
    function releaseJob(error) {
      ...
      client.msg_release(jobID, reservation_id, { delay }, function(releaseError) {
        if (releaseError)
