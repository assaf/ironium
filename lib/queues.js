      ...
      this._server.getReserveCapacity()
        .then(maxJobs => {
          if (maxJobs > 0) {
            ...
            // No capacity to consume jobs right now.
        ...
        .then(jobOrJobs => {
          ...
          if (this._processing) {
            this._server.incrementActiveJobs(jobs.length);
            jobs.forEach(job => {
...
module.exports = class Queues {
  ...
  // Returns the maximum amount of jobs a queue should try to reserve.
  // Every queue gets a share of reservation capacity to better deal
  // with load spikes across queues.
  getReserveCapacity() {
    return this.configPromise
      .then(config => {
        const totalCapacity       = config.concurrency - this._activeJobs;
        const maxCapacityPerQueue = Math.floor(totalCapacity / this._queues.size);
