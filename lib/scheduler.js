...
module.exports = class Scheduler {
  ...
  queueJob(jobName) {
    ...
    return this._canScheduleJob(jobName, time)
      ...
      .catch(error => {
        this._ironium.reportError(jobName, error);
        return Promise.reject(error);
      });
  }
  ...
  _runQueuedJob(job) {
    ...
    if (schedule) {
      // Skip recurring jobs outside of their run window, most
      // commonly because the job failed and was returned
      // to the queue.
      const tooOld = jobIsTooOld(schedule, job);
      if (!tooOld)
        return schedule._runJob(jobName, time);
      else {
        debug('Skipping this occurrence of %s', jobName);
        return Promise.resolve();
      }
    }
    else {
...
function jobIsTooOld(schedule, job) {
  if (schedule.every) {
    const jobTime        = +new Date(job.time);
    const nextOccurrence = schedule._getNextSchedule();
    const lastOccurrence = nextOccurrence - schedule.every;
    const tooOld         = jobTime < lastOccurrence;
    return tooOld;
  } else {
    // One-offs never get old.
    return false;
  }
}
